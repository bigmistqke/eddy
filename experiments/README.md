...
| 29 | [full-lifecycle](29_full-lifecycle/README.md) | C2 architecture end-to-end at K=16: load (cold-start) → play 10s → record 5s (with playback) → save (RGBA→AV1 parallel encode). Integration test — does the whole flow hold together in one continuous run? | **Yes, end-to-end** — total 23.3s: LOAD 2.85s, PLAY 58.7fps/0.9%, RECORD 57fps/3.2%+capture, SAVE 4.25s (~2× slower than 27 due to OPFS read). No cumulative issues |
