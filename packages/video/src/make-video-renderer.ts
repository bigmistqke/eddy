 ...
 * Make Video Renderer
 *
 * WebGL-based video frame rendering. Renders VideoFrames to an OffscreenCanvas
...
const log = debug('video:make-video-renderer', false)
...
/**
 * Create a renderer for an OffscreenCanvas.
 * Handles texture management and rendering. Effect compilation is external.
 */
export function makeVideoRenderer(canvas: OffscreenCanvas): VideoRenderer {
  ...
  log('VideoRenderer initialized', { width: canvas.width, height: canvas.height })
