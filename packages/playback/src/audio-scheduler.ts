...
import { createAudioRingBuffer, createRingBufferWriter, type AudioRingBuffer } from './audio-ring-buffer'
...
export interface AudioSchedulerOptions {
  ...
  channels?: number
  /**
   * External ring buffer SharedArrayBuffers.
   * If provided, uses these instead of creating new ones.
   * Useful when a worker writes directly to the ring buffer.
   */
  ringBuffer?: {
...
export interface AudioScheduler {
  ...
  /** The ring buffer's sample SharedArrayBuffer (for worker access) */
  readonly sampleBuffer: SharedArrayBuffer

  /** The ring buffer's control SharedArrayBuffer (for worker access) */
...
export async function createAudioScheduler(
  ...
  // Use external ring buffer if provided, otherwise create our own
